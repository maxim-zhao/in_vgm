...
calc (OPLL * opll)
{
  ...
  else
  {
    ...
    if (!(opll->mask & OPLL_MASK_CYM) && (CAR(opll,8)->eg_mode != FINISH))
      perc -= calc_slot_cym (CAR(opll,8), MOD(opll,8)->pgout);
  }
...
{
	calcPanning( opll->pan[ch & 15], pan ); // Maxim
}
